...
func (w *Worker) processMissingGTasks(gtasks, ltasks tasks.Tasks) error {
	...
	for _, task := range ltasks {
		...
		go func(t *tasks.Task) {
			...
			err = w.repo.UpdateGoogleID(t.ID, t.GoogleID)
			if err != nil {
...
func (w *Worker) processMissingLocalTasks(gtasks, ltasks tasks.Tasks) error {
	var wg sync.WaitGroup
	...
	for _, gtask := range gtasks {
		...
		if task.Deleted || gtask.Deleted {
			...
			go func(tId int, gtId string, tDel, gtDel bool) {
				...
				if tDel {
					err := w.gApi.DeleteTaskByID(gtId)
					if err != nil {
...
func (w *Worker) SyncGTasks() error {
	var wg sync.WaitGroup
	...
	go func() {
		...
		gtasks, err = w.gApi.GetAllTasksWithDeleted()
		if err != nil {
	...
	if err = w.processMissingGTasks(gtasks, tasks); err != nil {
		return fmt.Errorf("failed to process missing google tasks: %w", err)
	}

	if err = w.processMissingLocalTasks(gtasks, tasks); err != nil {
